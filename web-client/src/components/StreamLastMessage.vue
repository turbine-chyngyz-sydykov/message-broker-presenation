...
<template>
    <div class="p-6 bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
        <a href="#">
            <h2 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Stream Last Message</h2>
        </a>
        ...
        <div v-if="socket" class="flex mt-4 relative">
            <button type="button" @click="sendCommand()"
                    class="text-white bg-blue-700 font-medium rounded-lg text-sm px-2 py-2">
                    Get last message
            </button>
            <span class="bg-green-100 text-green-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-green-200 dark:text-green-900">Waiting: {{this.waiting}}</span>
        </div>
...
export default {
    name: "StreamLastMessage",

    data() {
        return {
            socket: null,
            channel: "stream-last-message-channel",
            messages: [],
            message: '',
            waiting: 0,
        }
    ...
    methods: {
        sendCommand() {
            if (this.socket) {
                this.socket.send('get_last_message');
            }
            this.waiting++;
        },
        getMessage(message) {
